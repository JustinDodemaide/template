...
@export var crouch_ceiling_detection:RayCast3D
...
func _physics_process(delta): # Most things happen here.
	...
	if not player.is_on_floor() and gravity and gravity_enabled:
	...
	low_ceiling = crouch_ceiling_detection.is_colliding()
	...
	was_on_floor = player.is_on_floor() # This must always be at the end of physics_process
...
func play_jump_animation():
	...
	if !was_on_floor and player.is_on_floor(): # The player just landed
		...
		var velocity_2D : Vector2 = Vector2(player.velocity.x, player.velocity.z).normalized()
...
func update_debug_menu_per_tick():
	...
	current_speed = Vector3.ZERO.distance_to(player.get_real_velocity())
	# $UserInterface/DebugPanel.add_property("Speed", snappedf(current_speed, 0.001), 1)
	# $UserInterface/DebugPanel.add_property("Target speed", speed, 2)
	var cv : Vector3 = player.get_real_velocity()
	var vd : Array[float] = [
		snappedf(cv.x, 0.001),
		snappedf(cv.y, 0.001),
		snappedf(cv.z, 0.001)
